...
# Binary Cross-Entropy(BCE)는 이진 분류(0또는 1) 문제에서 모델의 예측 확률값과 실제값 사이의 오차를 계산
model.compile(
...
# z = w.x + b -> sigmoid(z) -> BCE계산 -> gradient(y^ - y) -> w,b갱신

# 예측값과 실제값으로 시각화(S 곡선 형태)
import matplotlib.pyplot as plt

# 2차원 입력(x1, x2)을 가진 모델을 1차원 처럼 만들어 시그모이드(S곡선)를 보기위한 준비
...
m_eval2_multi = multi_model.evaluate(
    [x1_data, x2_data],
    y_data,
    verbose=0
)
