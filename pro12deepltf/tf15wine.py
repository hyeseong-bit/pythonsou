...
print(f"전에 훈련되지 않은 모델의 정확도:{acc * 100}%")

# 학습 모델

# 조기 종료
elrly_stop = EarlyStopping(monitor='val_loss', patience=5, restore_best_weights=True)
...
history = model.fit(x_train,y_train, epochs=1000,\
                    validation_split=0.2, batch_size= 64,\
                        callbacks=[elrly_stop, chkpoint])

# 모델의 정확도
loss, acc = model.evaluate(x_train, y_train, verbose=0)
print(f"학습 후 모델의 정확도:{acc * 100}%")
