...
# 두 집단 이상에서 각 범주 집단 단의 비율이 서로 동일한가를 검정하게 된다
# 두 개 이상의 범주형 자료가 동일한 분포를 갖는 모집단에서 추출된 것인지 검정하는

# 동질성 검정실습1 :
# 교육방법(독립변수)에 따른 교육생들의 만족도(종속변수) 분석 동질성 검정하는 방법이다
# survey_method csv
...
# 연령대별 sns 서비스별 이용률 현황은 동일하지 않다. 의견을 인정함
...
samp_data = pd.crosstab(index=samp_data["age"], columns=samp_data['service'])
print(samp_data)

chi2, p, dof, expected = stats.chi2_contingency(ctab2)
print(f"chi2:{chi2}, p:{p}, dof:{dof}")
# chi2:102.75202494484225, p:1.1679064204212775e-18, dof:8tg