...
data = frame.iloc[:, [0,1,7,8]]  # 'YMD', 'AMT', 'CNT', 'maxTa', 'sumRn'
print(data.head())
print("결측치 확인 : ", data.isnull().sum())

# 독립표본 t검정 ---
# print(data["sumRn"] > 0) # 강수령이 조금이라도 있으면 True
...
print('tg2(비가 오는 날에) 매출액 평균: ',np.mean(tg2)) # 757331.5217391305
...
print(stats.shapiro(tg1).pvalue)   # 0.05605 > 0.05 만족
print(stats.shapiro(tg2).pvalue)   # 0.88275 > 0.05 만족

# 등분산 섬정
print(stats.levene(tg1, tg2).pvalue)  # 0.71234523 > 0.05 만족

print(stats.ttest_ind(tg1, tg2, equal_var=True))
# statistic=0.101098, pvalue=.91953, df=326.0
# 해석 : 정규성, 등분산성 조건은 충족함.
