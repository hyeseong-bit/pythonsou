...
# 세 집단 이상의 평균비교에서는 독립인 두 집단의 평균 비교를 반복하여 
# 실시할 경우에 제1종 오류가 증가하게 되어 문제가 발생한다.
...
# 실습) 세 가지 교육방법을 적용하여 1개월 동안 교육받은 교육생 80명을 
...
# ols(Ordinary Least Sqares - 최소제곱법 회귀)
...
result = sm.stats.anova_lm(lin_model, type = 1)
print(result)

#               df(자유도) sum_sq(제곱합) mean_sq(제곱평균)  F(F값)      PR(>F)
# data['method']   1.0     27.980888   27.980888           0.122228    0.727597
# Residual        76.0  17398.134497  228.922822             NaN         NaN
# 해석 : p-value 0.727597 > aphah 0.05 이므로 귀무 채택
...
# 정확히 어느 그룹의 평균값이 의미가 있는지를 알려주지 않는다.
...
#      1      3   1.4904 0.9363 -8.8183  11.799  False : 유의미한 차이가 있으면 Ture
