...
def dijkstraFunc(graph, start):
    ...
    while pq:    # 처리할 노드가 남아 있는 동안
        ...
        if cur_dist > dist[u]: # 더 짧은 경로가 있으면 현재 노드는 무시(중복)
            continue

        for v, w in graph[u]:           # 현재노드 u와 연곃된 이웃 노드 v탐색
            new_dist = cur_dist + w     # u를 거쳐 v(새노드)로 가는 새 거리 계산
            if new_dist < dist[v]:
                dist[v] = new_dist       # 최던 거리 저장소(최단 거리 갱신)
