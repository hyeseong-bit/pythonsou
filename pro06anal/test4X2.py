# <선호도 분석 실습>
# 일원카이제곱 검정
# 5개의 스포츠 완료에 대한 선호도에 차이가 있는지 검정하기
# 귀무가설 : 기대치와 관찰지는 차이가 없다. 스포츠 음료의 선호도에 차이가 없다.
...
# 판정 : 유의수준 0.05, < pvalue= 0.00039991 이므로 귀무기각
# 스포츠 음료의 선허도에 차이가 있다. 라는 의견이 받아들여짐
...
plt.bar(x + width / 2, data['관측도수'], width=width , label='기대도수',alpha=0.6)
