...
print('\n단순선형회귀모델이므로 적절성 선행조건 중 잔차의 정규성, 선형성 확인')
# 잔차(Residual): 실제관측값과 모델이 예측한 값의 차이를 의미한다.
...
print('잔차평균값:', np.mean(residual[:5])) #  1.6738829920227805
...
print(f"breuschpagan test : 통계량 :{bp_test}, p-value:{bp_pvalue}")
print("등분산성 만족" if bp_pvalue > 0.05 else "등분산성 만족")

print()
# 참고 : Cook's distance - 특정 데이터가 회귀모델에 얼마나 영향을 주는지 확인
# 영향력 있는 관측지(이상치)를 탐지하는 진단 방법
