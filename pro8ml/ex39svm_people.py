...
# plt.imshow(faces.images[1], cmap='bone')  # 이미지 1 개 시각화
# plt.show()
...
# whiten=Ture : 주성분의 스케일이 작아지도록 조정
x_low = m_pca.fit_transform(faces.data)  # (1346, 2914) -> (1348, n)
print('x_low : ', x_low, ' ', x_low.shape)  # (1348, n만큼)

fig, ax = plt.subplots(3, 5, figsize=(10, 6))
for i, axi in enumerate(ax.flat):
    axi.imshow(m_pca.components_[i].reshape(faces.images[0].shape), cmap='bone')
    # reshape(feaces.images[0].shape) : [2914] -> [62, 47]
    axi.axis('off')
    axi.set_title(f'PC {i + 1}')
plt.suptitle('Eigenfaces(주성분 얼굴)', fontsize=12)
plt.tight_layout()
plt.show()      # 출력 이미지는 실제 얼굴이 아니라 특징 패턴을 보여줌(얼굴 윤곽, 눈위치, 코 , 그림자 ...)을 보여줌
# SVM 알고리즘은 실제 얼굴이 아니라 특징 패턴으로 분류작업을 한다.

print('설명력 확인')
print(m_pca.explained_variance_ratio_[:10]) 
# [0.20795521 0.13552678 0.06788548 0.0574195  0.04768274 0.02769716
#  0.02342142 0.01973505 0.01856109 0.01784673]
print('누적 설명력 : ', m_pca.explained_variance_ratio_.sum())  # 0.9039584
...
print('\n\n새로운 이미지를 입력해 분류하기 ----')
# 현재 모델의 분류 accuracy :  0.807121

# 실습 1 : 기존 데이터로 테스트
test_img = faces.data[0].reshape(1, -1)  
print('test_ing : ', test_img)          # [[0.53464055 0.5254902  0.49673203
test_pred = mymodel.predict(test_img)
print('실습1 예측 결과 : ', faces.target_names[pred[0]], ', index:',test_pred[0])
print('실제값 : ', faces.target_names[faces.target[0]], ', index:', faces.target[0])
# 실습1 예측 결과 :  George W Bush , index: 1
# 실제값 :  Colin Powell , index: 1

print()
# 실습 2: 새로운 데이터로 분류하기 (이게 찐또배기)
# 단계 : 이미지 읽기 -> 크기 맞추기(62 x 47) -> 1차원으로 변환 -> 예측
from PIL import Image

img = Image.open('bush.jpeg')   # 이미지 읽기
img = img.convert('L')   # 흑백으로 변환  
img = img.resize((47, 62))    # (width, height) 크키맞추기
# numpt 이미지는 (height, width) - 세로 가로
# PIL 이미지는 (width, height)  - 가로, 세로. 이미지는 라이브러리 마다 축 순서가 다름.
img_np = np.array(img)  # numpy 변환
# print('img_np : ', img_np)  # [[[ 83  83  83] ... 0 ~ 255 숫자로 구성
img_np = img_np / 255.0 # 정규화(학습 데이터와 맞춰주어야 함)
# print('img_np : ', img_np) #   [[[0.3254902  0.3254902  0.3254902 ][0.3254902  0.3254902  0.3254902 ] ...
