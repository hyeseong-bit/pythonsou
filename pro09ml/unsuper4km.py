...
# k-평균 군집화(M-means Clustering) 알고리즘은 군집의 중심이 되는 k개의 seed 점들을 선택하여
...
#  - 군집의 변화가 없을때
#  - 중심점의 이동이 임계값 이하일 때
#  - 왜곡값(distortion, 각ㅇ각의 클러스터의 거리제곱의 총합)
...
init_centroid = 'k-means++'    # cluster의 중심을 k_means++로 선택. 중심을 최대한 멀리
kmodel = KMeans(n_clusters=3, init=init_centroid, n_init=10, random_state=0)
# n_init10 : KNeans를 10회 실행 - 가장 좋은 결과(오차 최소값)를 선택
...
plt.scatter(kmodel.cluster_centers_[:, 0], kmodel.cluster_centers_[:, 1],
            c = 'black', marker='+', s=60, label='center') # 각 군집 중심점 표시
