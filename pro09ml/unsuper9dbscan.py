...
# 잘 정의되지 않고 이상치가 많은 데이터를 처리할 때 유용하게 사용 가능하다.
...
# 샘플 데이터 생성
x, y = make_moons(n_samples=200, noise=0.05, shuffle=True)
...
print('DNSCAN으로 군집 분류')
db = DBSCAN(eps=0.2, min_samples=5, metric='euclidean')
# eps=0.2 - 샘플간 최대거리, min_samples= 5 - 점에 대한 이웃 샘플수(반경 포인트들의 최소)
