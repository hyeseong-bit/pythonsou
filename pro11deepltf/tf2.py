...
# autograph 기능 : 파이썬 코드를 텐서플로 그래프(Graph) 코드로 자동 변환
# 텐서플로의 두 가지 실행방법
# 1) Eager Execution : 파이썬 코드 처럼 즉시 실행 (기본)
# 2) Graph Execution : 계산 그래프를 만들어 최적화 후 실행 (텐서 처리에 효율적)
# 위 ft.cond()를 AutoGraph를 사용한 경우

@tf.function            # AutoGraph가 개입함 (텐서플로 그래프를 연산을 함)
def calcFunc1(a, b):    # 위 ft.cond()를 AutoGraph 사용한 경우
    if (a < b):         
        return tf.add(10, a)
    else:
        return tf.square(a)
...
def calcFunc3():    # 1부터 3까지 증가
    ...
    for _ in tf.range(3):
        ...
        imsi = tf.add(imsi, su) # 파이썬 연산자(권장)
