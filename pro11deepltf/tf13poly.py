...
# 다항회귀에 적합한 데이터생성 -> CSV 파일로 저장 후 읽기 -> 산점도
...
# 모델 성능 평가 함수
def evalue_model(name, y_true, y_pred):
...
# 성능 비교
evalue_model('선형회귀', y_test, y_pred_linear)
evalue_model('다항회귀(degree=2)', y_test, y_pred_poly)
...
plt.plot(x_plot, y_plot_poly, label='다항회귀(degredd=2)')
plt.xlabel('광고비')
plt.ylabel('매출')
plt.legend()
plt.grid(True)
