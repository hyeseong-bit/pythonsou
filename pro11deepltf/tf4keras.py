# 다층 신경망 구성 : Keras(Deep Learnung lib) 모듈 사용
...
model.compile(loss='binary_crossentropy', optimizer='sgd', metrics=['accuracy'])
    # loss(손실함수): 훈련 데이터에서 신경망의 성능을 측정하는 방법으로 모델이 정확히 학습할 수 있다.
...
model.fit(x=x, y=y, epochs=1, batch_size=1, verbose=1)
...
mymodel = load_model('tf5model.keras')
...
# 역전파 Backpropagation
# 모델의 예측값이 정답과 얼마나 다른지 계산한 뒤, 그 오차를 거꾸로거슬러 올라가며
...
# 한마디로 역전파는 손실ㄲ밧을 기준으로 각 가중치의 책임 정도를 계산하고, 그 결과를 이용해
