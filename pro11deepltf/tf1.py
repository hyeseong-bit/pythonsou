...
print(12, type(12)) #  <class 'int'> 파이썬 상수로 파이썬이 직접 계싼
print(tf.constant(12))    # tf.Tensor(12, shape=(), dtype=int32) 0d텐서(scaler)
print(tf.constant([12]))   # tf.Tensor([12], shape=(1,), dtype=int32) 0d텐서(vector)
print(tf.constant([[12]]))   # tf.Tensor([[12]], shape=(1, 1), dtype=int32) 2d텐서(matrix)
...
print()
imsi = np.array([1, 2]) # 일반 수치 연산(CPU연산이 기본, 자동 미분 불가)
...
tf.print(tf.random.normal([3], 0, 1)) # 균등분포 ([shape],평균,표준편차)
