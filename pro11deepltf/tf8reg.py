...
outputs = Dense(units=4, activation='linear')(output1)
...
print('\n모델 생성 방법3 - Sub classing : Model을 상속 받아 직접 모델을 생성')
class MyModel(Model):
    def __init__(self):
    ...
    # x : input 매개변수
    def call(self, x):  # Input 클래스를 사용하지 않고 call 메소드의 input 매개변수
        x = self.d1(x)
        return self.d2(x)
...
class MyLayer(Layer):
    ...
    def build(self, input_shape):
        ...
        self.w = self.add_weight(shape=(input[-1], self.units), \
