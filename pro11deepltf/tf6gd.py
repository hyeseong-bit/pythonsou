# 비용함수(Cpst Function)는 머신러닝 모델의 예측값과 실제정답의 차이(오차)를
# 수치화하여 모델의 성능을 평가하는 함수입니다.
# 목적은 이 비용 함수의 값을 최소화하는 파라미터를 찾는 것입니다.
# 수식은
# 인공 신경망은 델타규칙(경시하강법)으로 W(weight)와 B(bias)를 갱신한다.
...
# wx + b 수식에서 w와 b를 최적의 추세선이 만들어지도록 갱신해여한다.
...
# 선형회귀 모델 수식 hypotgesis = w * x + b
# cost = tf.reduce_sum(tf.pow(hypotgesis - y, 2)) / len(y)
