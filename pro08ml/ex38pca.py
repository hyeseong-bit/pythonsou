...
# 이렇게 구한 고유벡터에 입력 데이터를 선형변환 하는 것 이다.
# 이 고유백터가 PCA의 주성분 벡터로서 입력 데이터의 분산이 큰 방향을 나타낸다.
...
# 순서1 : 입력 데이터의 공분산 행령을 생성한다.
# 순서2 : 공분산 행렬의 고유벡터 고윳값(고유백토 크기)을 계산한다.
# 순서3 : 고유값이 큰 순서대로 k개(PCA 변환 창수 만큼) 만큼 고유벡터 추출
...
df = pd.DataFrame(x)
ax = sns.scatterplot(x=0, y=1, data=df, markers='s', s=100, color='b')
# 각 점에 대해 text 표시
for i in range(n):
    ax.text(x[i, 0] - 0.05, x[i, 1] + 0.03, '표본{}'.format(i + 1))
...
print('model2 accuracy : ', metrics.accuracy_score(label, pred2)) # 0.946666
