...
#  LogisticRegression : 다중 클래스(label, 정속변수)를 지원하도록 일반화 됨
...
# 모델에 패널티 적용1 ==> C:L2규제. 숫자값을 조정해 가며 정확도 확인
...
print('분류 정확도 확인 1')
print(f'{accuracy_score(y_test, y_pred)}')   # 0.9777777

print('분류 정확도 확인 2')
con_mat = pd.crosstab(y_test, y_pred, rownames=['예측치'], colnames=['관측지'])
print(con_mat)
print(con_mat[0][0] + con_mat[1][1] + con_mat[2][2] / len(y_test))  # 0.9777777

print('분류 정확도 확인 3')
print('test score : ', model.score(x_test,y_test))     # 0.977777777
print('train score : ', model.score(x_train,y_train))  #  0.9714285
...
# 위의 결과는 softnmax의 확률값 중 가장 큰 인덱스가 출력된 값이다
